...
def render_ascii_to_image(ascii_str, font_path=None, font_size=10):
    """Render ASCII string as PNG image (strip ANSI codes for PNG)."""
    # Remove ANSI codes for image rendering
    import re
...
class ASCIIConverterApp:
    def __init__(self, root):
        ...
        ttk.Label(self.frame, textvariable=self.width_var).grid(
            row=2, column=3)
        ...
        ttk.Label(self.frame, textvariable=self.height_var).grid(
            row=3, column=3)

        self.color_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(self.frame, text="Use Color (Terminal)",
                        variable=self.color_var).grid(row=4, column=0, columnspan=2)
    ...
    def process_video(self, is_webcam):
        ...
        if (self.mp4_var.get() and not is_webcam) or (self.mp4_var.get() and is_webcam):

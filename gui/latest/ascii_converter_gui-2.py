...
def render_ascii_to_image(ascii_str, font_path=None, font_size=10):
    ...
    width = max(font.getbbox(line)[2]
                for line in lines if line.strip()) if lines else 100
...
class ASCIIConverterApp:
        ...
        try:
            ...
            if self.png_var.get():
                img, _, _ = render_ascii_to_image(
                    ascii_art, font_path=self.font_path, font_size=self.font_size_var.get())
    ...
    def process_video(self, is_webcam):
        ...
        try:
            ...
            fps = max(1, self.fps_var.get())  # Ensure FPS is at least 1
